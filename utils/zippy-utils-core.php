...
class Zippy_Utils_Core
{
  ...
  {
    $encryption_key = ZIPPY_ADDONS_PREFIX;
  ...
  {
    return array(
      'id'          => ZIPPY_ADDONS_PREFIX . '_divider',
      'name'       => __('', ZIPPY_ADDONS_PREFIX . 'woocommerce-settings-tab'),
      'type'        => 'title',
      'desc' => '<hr>'
    );
  }
  ...
  public static function get_template($template_name, $vars = array(), $absolute_path = '', $relative_path = '')
  {
    ...
    if (file_exists(ZIPPY_ADDONS_DIR_PATH . $relative_path . $template_name)) {
      $template = ZIPPY_ADDONS_DIR_PATH . $relative_path . $template_name;
    }

    $template = apply_filters_deprecated(ZIPPY_ADDONS_PREFIX . '\util\get_template\path_file', [$template, $vars], '1.0.0', ZIPPY_ADDONS_PREFIX . '\util\get_template\template');
    $template = apply_filters(ZIPPY_ADDONS_PREFIX . '\util\get_template\template', $template, $template_name, $absolute_path, $relative_path);
